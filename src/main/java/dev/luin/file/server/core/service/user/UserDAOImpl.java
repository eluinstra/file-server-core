...
class UserDAOImpl implements UserDAO
{
	...
	ConstructorExpression<dev.luin.file.server.core.service.user.Certificate> certificateProjection = Projections.constructor(dev.luin.file.server.core.service.user.Certificate.class, certificateTable.userId, certificateTable.certificate, certificateTable.timestamp);
		...
		try
		{
			return Option.of(
					queryFactory.select(userProjection)
							...
							.on(userTable.id.eq(certificateTable.userId))
	...
	@Override
	public long deleteUser(@NonNull final UserId userId)
	{
		return queryFactory.delete(userTable).where(userTable.id.eq(userId)).execute();
	}

	@Override
	public Seq<Certificate> selectCertificates(@NonNull UserId userId)
	{
		return List.ofAll(queryFactory.select(certificateProjection).from(certificateTable).where(certificateTable.userId.eq(userId)).fetch().stream()
				.map(c -> c.getCertificate())
				.toList());
	}

	@Override
	public long insertCertificate(@NonNull UserId userId, @NonNull Certificate certificate)
	{
		return queryFactory.insert(certificateTable)
				.set(certificateTable.userId, userId)
